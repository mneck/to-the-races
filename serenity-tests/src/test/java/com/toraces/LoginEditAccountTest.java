...
public class LoginEditAccountTest {
    ...
    private static final Target EDIT_NAME_INPUT = Target.the("edit name input")
        .located(By.cssSelector("[data-testid='edit-name-input']"));
    ...
    @Test
    void loginAndEditAccount() {
        String email = "edit" + randomSuffix + "@example.com";
        
        // Register user
        user.wasAbleTo(
            Open.url("http://localhost:3000/register"),
            Enter.theValue("Original Name").into(NAME_INPUT),
            Enter.theValue("edituser" + randomSuffix).into(USERNAME_INPUT),
        ...
        user.wasAbleTo(
            Enter.theValue("Updated Name").into(EDIT_NAME_INPUT),
            Click.on(SAVE_PROFILE_BUTTON)
        );
        
        // Verify update
        user.attemptsTo(
            Ensure.thatTheWebPage().containsText("Updated Name")
        );
    }
}
